...
export default function SearchAndSidebar({ articles, featured, latestNews, trendingServers, trendingSkills }) {
  ...
  return (
    <>
      {/* 1. UPPER SECTION: Unified & Properly Sized Search Bar */}
      <div style={{ maxWidth: '640px', margin: '0 auto 24px auto', position: 'relative' }}>
        <div
          style={{
            display: 'flex',
            alignItems: 'center',
        ...
        >
          ...
          >
            <span style={{ fontSize: '15px' }}>🏠</span> SKILLS
          </button>

          <div style={{ width: '1px', height: '20px', backgroundColor: '#ddd6fe' }} />
          ...
          >
            <span style={{ fontSize: '15px' }}>🔌</span> mcp servers
          </button>
          ...
          >
            <span style={{ fontSize: '15px' }}>🤖</span> AI tools
          </button>
      ...
      <div style={{ textAlign: 'center', marginBottom: '24px' }}>
        <h1 style={{ fontSize: '32px', fontWeight: '900', color: '#0f172a', margin: '0 0 8px 0', letterSpacing: '-0.02em' }}>
          Supercharge your AI with MCP & Skills
        </h1>
        <p style={{ fontSize: '15px', color: '#64748b', margin: 0 }}>
          Connect Claude, Cursor, and Windsurf to tools, APIs, and domain-specific agent skills.
        </p>
      </div>

      {/* 3. STREAMLINED & COMPACT POPULAR AI TOOLS (Replaces Section 1) */}
      <div style={{ marginBottom: '32px', backgroundColor: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '14px 18px', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.02)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
          <h3 style={{ fontSize: '14px', fontWeight: '800', color: '#0f172a', margin: 0 }}>🛠️ Popular AI Tools</h3>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '8px' }}>
          {POPULAR_AI_TOOLS.map((tool, idx) => (
            <Link key={idx} href={tool.link} style={{ textDecoration: 'none' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 10px', borderRadius: '10px', backgroundColor: '#f8fafc', border: '1px solid #f1f5f9', transition: 'border-color 0.15s ease' }}>
                <span style={{ fontSize: '16px' }}>{tool.icon}</span>
                <div style={{ overflow: 'hidden' }}>
                  <div style={{ fontSize: '12px', fontWeight: '700', color: '#0f172a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{tool.name}</div>
                  <div style={{ fontSize: '10px', color: '#64748b', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{tool.category}</div>
                </div>
              </div>
            </Link>
          ))}
