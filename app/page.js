...
export default function HomePage() {
  ...
  const slugs = useMemo(() => [...rawSlugs].reverse(), [rawSlugs]);
  ...
  const searchIndex = useMemo(() => {
    const servers = TRENDING_MCP_SERVERS.map(s => ({
      title: s.name,
    ...
    const skills = TRENDING_CLAUDE_SKILLS.map(sk => ({
      title: sk.name,
            ...
            {searchResults.length > 0 ? (
              searchResults.map((item, idx) => (
                <Link key={idx} href={item.link} style={{ textDecoration: 'none', color: 'inherit' }}>
                  <div style={{ padding: '12px 16px', borderRadius: '8px', cursor: 'pointer', transition: 'background 0.2s', borderBottom: '1px solid #f1f5f9' }} onMouseOver={(e) => e.currentTarget.style.backgroundColor = '#f8fafc'} onMouseOut={(e) => e.currentTarget.style.backgroundColor = 'transparent'}>
        ...
        <aside style={{ width: '320px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '28px' }}>
          ...
          <div style={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '20px', padding: '20px', boxShadow: '0 4px 12px rgba(0,0,0,0.02)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {TRENDING_MCP_SERVERS.map((server, idx) => (
          ...
          <div style={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '20px', padding: '20px', boxShadow: '0 4px 12px rgba(0,0,0,0.02)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {TRENDING_CLAUDE_SKILLS.map((skill, idx) => (
